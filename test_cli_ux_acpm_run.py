...
def test_acpm_run_help_shows_arguments():
    rc, out, _err = _run_cli("acpm", "run", "--help")
    assert rc == 0
...
def test_acpm_run_unknown_campaign_exits_1():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "DOES_NOT_EXIST", "--profile", "Balanced")
    assert rc == 1
    assert "Campaign not found" in out


def test_acpm_run_unknown_profile_exits_1():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "NGL_sweep", "--profile", "InvalidProfile")
    assert rc == 1
    assert "Unknown profile" in out


def test_acpm_run_unknown_tier_exits_1():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "NGL_sweep", "--profile", "Balanced", "--tier", "10x")
    assert rc == 1
    assert "Unknown tier" in out


def test_acpm_run_non_applicable_campaign_exits_1():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "B_low_sample", "--profile", "Balanced")
    assert rc == 1
    assert "not ACPM-applicable" in out


def test_acpm_run_validate_exits_0():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "NGL_sweep", "--profile", "Balanced", "--validate")
...
def test_acpm_run_dry_run_exits_0():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "NGL_sweep", "--profile", "Balanced", "--dry-run")
    assert rc == 0
...
def test_acpm_run_tier_3x():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "NGL_sweep", "--profile", "Balanced", "--tier", "3x", "--dry-run")
    assert rc == 0
    assert "3x" in out or "3" in out


def test_acpm_run_tier_5x():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "NGL_sweep", "--profile", "Balanced", "--tier", "5x", "--dry-run")
    assert rc == 0
    assert "5x" in out or "5" in out


def test_acpm_run_profile_ts():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "NGL_sweep", "--profile", "T/S", "--dry-run")
    assert rc == 0
    assert "T/S" in out


def test_acpm_run_profile_ttft():
    rc, out, _err = _run_cli("acpm", "run", "--campaign", "NGL_sweep", "--profile", "TTFT", "--dry-run")
    assert rc == 0
    assert "TTFT" in out


def test_manual_run_still_works():
    rc, out, _err = _run_cli("run", "--help")
    assert rc == 0
    assert "--campaign" in out
    assert "--mode" in out
...
def test_acpm_run_appears_in_acpm_help():
    rc, out, _err = _run_cli("acpm", "--help")
    assert rc == 0
    assert "run" in out