...
def test_runner_persists_acpm_recommendation_record(tmp_path: Path):
    from src.db import get_connection, init_db, read_recommendation_record
    from src.runner import _persist_acpm_recommendation_record
    ...
    run_plan = RunPlan(
        ...
        effective_campaign_id="NGL_sweep",
        run_mode="full",
        variable="n_gpu_layers",
        all_campaign_values=[10, 20, 30, 40, 50, 60, 70, 80, 90, 999],
    ...
    record = _persist_acpm_recommendation_record(
        db_path=db_path,
        campaign_id="NGL_sweep",
        run_plan=run_plan,
        scores=_score_result(winner="NGL_sweep_999"),
    ...
    assert record["status"] == "best_validated_config"

    with get_connection(db_path) as conn:
        persisted = read_recommendation_record(conn, "NGL_sweep")

    assert persisted is not None
    assert persisted["recommended_config_id"] == "NGL_sweep_999"
